...
Choose **Native patrol**, **Idle**, or **Route**. Blue lines show a sampled native patrol, not a guaranteed loop: original behaviour reacts to the player and some guards have no roaming path. Shift-click the scene to add up to 16 route points, then drag their handles; custom routes loop back to spawn and respect room boundaries. Only one guard engages at a time; others wait during combat and do not collide with each other. Defeated guards remain down for that play session. Modified OFF restores native behaviour. Save/load, undo/redo and Restore all include enemy edits; **Original enemies** restores only the room's original enemy setup. Test room uses the edits when Modified is ON.

`--enemy-editor-test` checks catalogs across all levels, multiple guards in each game, exclusive engagement, patrol movement, room persistence, scene file save/load and actual gameplay save/rewind restoration. These automated checks do not replace a manual playthrough of custom layouts.
