...
def main():
    ...
    status=dict(schema=1,project='LNPreserve',stage='native_conversion_in_progress',
        ...
        pending=['Complete and verify LN1 puzzle sequences, special enemies, projectile combat, palette effects, death presentation and ending',
            'Complete LN2 boundary hazards, swarm and remaining object details; verify projectile display timing, the final encounter and ending with original input replays',
    ...
    status['native_original_routines'].extend([
        dict(name='LN1 dungeon spider',verification='1024 original approach/steering/capture states; 88 dungeon special pose PNGs regenerated by original drawing code'),
        dict(name='LN1 ordinary enemy colours',verification='4096 original compositor selections shared by all six banks; 3121 unique PNG frames'),
        dict(name='LN2 projectile combat',verification='14336 original spawn/update states, including ammunition, lifetimes and damage; full input/display timing unverified'),
        dict(name='LN2 projectile masking',verification='2976 original 63-byte mask probes and 1499904 GPU pixels; includes odd alignment and edges'),
        dict(name='LN2 bodies during throws',verification='300 unmasked original compositions; 2764800 GPU pixels; full masked body/raster parity unverified')])
    status['ln2_projectiles']=dict(original_projectile_pngs=18,unique_body_part_pngs=1754,
        original_full_playthrough_verified=False,full_display_timing_verified=False)
