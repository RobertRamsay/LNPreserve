...
function ln3_play_tick(_g,_joy) {
    ...
    var _timers=["logic_wait","enemy_attack_wait","weapon_notice_timer","scene_wait","regeneration_wait","death_wait","item_wait","special_wait"];
...
function ln3_play_actor_part(_g,_d,_i) {
    ...
    var _bank=asset_get_index(_g.world.actor_bank),_colour=_g.palette[_d.draw_colours[_i]&15];
...
function ln3_play_draw(_g) {
    ...
    for (var _order=0;_order<8;_order++) ln3_play_actor_part(_g,_g.display,_g.animation.order[_order]);
    ...
    draw_text(790,672,"Item "+string(_s.selected_item)+"   Weapon "+string(_s.player_weapon));
