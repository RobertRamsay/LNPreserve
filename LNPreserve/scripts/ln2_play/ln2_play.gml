function LN2Play(_level=1) constructor {
    ...
    player.reverse_roll_enabled=true; // Native enhancement; absent from original-state replays.
    ...
    status_data=ln3_data_read("play/ln2/status.json");
...
function ln2_test_enter(_g,_entry) {
    ln2_ending_free(_g);_g.ending_surface=-1;_g.victory=0;
...
function ln2_level_load(_g,_level,_ordinary=false) {
    ...
    ln2_ending_free(_g);
    if (_ordinary) repeat(10) ln2_score_add(_g,$50);
    ln2_enemy_remember(_g);
    var _flags=[];for (var _i=17;_i<26;_i++) array_push(_flags,_g.inventory[_i]);
    _g.level_states[_g.level-1]={enemies:_g.room_enemies,passages:_g.opened_passages,world:_g.world_state,flags:_flags};
    ...
    if (is_struct(_saved) && !_ordinary) { _g.room_enemies=_saved.enemies;_g.opened_passages=_saved.passages;_g.world_state=_saved.world; }
    // The original level reset clears the nine temporary flags at $03e9..$03f1.
    // Scene testing may restore a visited level's flags without mixing levels.
    for (var _i=17;_i<26;_i++) _g.inventory[_i]=is_struct(_saved) && !_ordinary?_saved.flags[_i-17]:0;
...
function ln2_play_tick(_g,_joy) {
    ...
    if (!_drowning) {_joy=ln2_basement_jump_input(_g,_joy);_joy=ln2_burger_input(_g,_joy);_joy=ln2_candle_assist_input(_g,_joy);_joy=ln2_pickup_assist_input(_g,_joy);}
    ln2_street_clock_tick(_g,_tick);
    ln2_sewer_flame_tick(_g,_tick);
    ln2_status_tick(_g,_tick);ln2_office_grate_tick(_g);ln2_mansion_alarm_tick(_g,_tick);
    _g.room_age++;
    ...
    if (_g.level==7 && _g.world_state.boss_defeated) ln2_victory_start(_g);
}
...
function ln2_play_actor(_g,_a,_enemy) {
    ...
    if (_enemy && _a.custom && ((_g.level==4 && _g.room_id==3 && _a.display_frame==99) ||
        (_g.level==5 && _g.room_id==9 && array_contains([99, 100, 101],_a.display_frame)))) {
        var _scenery=_g.level==4?spr_ln2_basement_trolley:spr_ln2_office_fan;
        var _frames=_g.level==4?[99]:[99, 100, 101],_index=0;
        for(var _i=0;_i<array_length(_frames);_i++) if(_frames[_i]==_a.display_frame) {_index=_i;break;}
        ln_draw_masked_actor(_scenery,_index*2+real(_a.mirror),_a.x,_a.y,1,1,_g.mask,0,0,240,144,max(0.001,(_a.depth_y-0.25)/255));return;
    }
    if (_enemy && _a.custom && ((_g.level==5 && _g.room_id==14 && _a.display_frame==102) || (_g.level==6 && _a.display_frame==99))) {
        ln_draw_masked_actor(_g.level==5?spr_ln2_level5_helicopter:spr_ln2_level6_helicopter,real(_a.mirror),_a.x,_a.y,
            1,1,_g.mask,0,0,240,144,max(0.001,(_a.depth_y-0.25)/255));return;
    }
    ...
    if (_extra && _enemy && !_a.custom && variable_struct_exists(_g.world,"enemy_extra_frames") && variable_struct_exists(_g.world.enemy_extra_frames,_key))
        _frames=variable_struct_get(_g.world.enemy_extra_frames,_key);
    if (_extra) for (var _i=0;_i<array_length(_frames);_i++) if (_frames[_i]==_a.display_frame) { _index=_i;break; }
    if (_index<0) return;
