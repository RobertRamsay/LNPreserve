level_map_test=false;
for(var _map_arg=1;_map_arg<=parameter_count();_map_arg++) if(parameter_string(_map_arg)=="--level-map-test") level_map_test=true;
...
ln_tool_init();
global.ln_tracks=new LNTrackPlayer();
global.ln_sprites=new LNSpriteViewer();
sprite_viewer_test=false;sprite_viewer_test_frame=0;
for(var _sprite_arg=1;_sprite_arg<=parameter_count();_sprite_arg++) if(parameter_string(_sprite_arg)=="--sprite-viewer-test") sprite_viewer_test=true;
track_player_test=false;
for(var _track_arg=1;_track_arg<=parameter_count();_track_arg++) if(parameter_string(_track_arg)=="--track-player-test") track_player_test=true;
painting_test=false;
rewind_test=false;
