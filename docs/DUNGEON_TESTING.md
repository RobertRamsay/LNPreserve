...
The original maze connections are retained. Several exits return to the same scene, and scene 15 has two exits facing SE. The arrow shortcut chooses the one nearest the player; move towards the desired doorway before pressing it.
...
From scene 16, SW reaches 19; SW again reaches 23. Arrow travel is a testing shortcut and bypasses normal puzzle requirements. Reachability checks and isolated source comparisons do not establish a complete original-input playthrough or cycle accuracy.
