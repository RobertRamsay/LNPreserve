...
Mechanisms pass 3,520 original routine states and sequence requests. Earth and Water resume after their original 54/45 PAL-tick fade waits. Void's reflected bolts lead through the native curtain/message sequence into scene 12 at the original position/facing from $69ea-$6a1a. The eight curtain sprites and three messages are original artwork; movement passes 1,260 original IRQ-helper states. Fade/wipe/message integration checks completion and final entrance state, not identical interrupt phase or complete VIC output. The final fight now runs the native ENDING picture sequence and scrolling text. Ten unique original picture/animation frames and 22 original font PNGs supply the artwork. The original RLE update costs schedule completed animated pictures; partial drawing during VIC display, IRQ stealing and raster reveal are not verified. The scroll passes 1,736 original states and 58,368 GPU pixel checks. The ending waits for J after the text and reaches the source high-score request boundary. That separate high-score program remains pending; the runtime displays the earlier original victory message at this boundary.
...
## LN2 final mechanisms and ending

The original $b337 keypad prerequisite and $b375 edge/submit rules now drive native digits and gate acceptance. The first-run code is the source's 1111 ($0297..$029a = $1c); Office reveals that stored code. The original later game-over code-regeneration path is separate and remains pending. The temporary native keypad panel omits the source's blocking pre-poll delay. A total of 4,096 original keypad states compare successfully.

The final enemy's initially inactive scene record supplies weapon 1/costume 2, which differs from the room-0 guard. Its original release ($b3fd) passes 384 states. Five candle flags and the original final-area/knockout predicate pass 3,840 states, including the alternate $9cb3 defeat response when all candles are lit before the last hit. Both $c1b9 and $c1c0 sequences have native art, including the previously missing pose 99. The final enemy's 208 pose/mirror images pass 1,916,928 unmasked GPU pixel checks. Fifteen original candle fragments pass 450 bitmap pixel samples; complete masked/raster composition is separate.

All seven banks now use their six source post-item animation continuations; 12,544 pointer/countdown comparisons pass. The level-local flags at $03e9..$03f1 clear during normal progression and are saved separately for F11 revisits. Common carried items and the stored keypad code survive travel. Score awards for scene visits, ordinary successful interactions, knockouts and level/victory bonuses are connected, with full event dispatch still requiring a playthrough audit. Original score arithmetic, elapsed-clock rollover and health-display state pass 6,144 comparisons; native dashboard presentation is still temporary.

The final flag enters the source's separate spirit loop instead of ordinary enemy decisions. The reward at item 23 leads to the congratulations loop after the spirit animation finishes. Original state checks cover 3,072 victory transitions, 2,048 colour-phase states and 2,048 centring-motion states, and a native integration test reaches the ending. The $8f90 colour substitution has a distinct initial traversal: 12 source bitmap states return to state 4 for the steady loop. Identical PNGs are shared. The original $8cff text/bitmap instructions supply the end screen, with native run results drawn using its original digit font. GPU checks cover 30,720 game/ending bitmap samples. The saved ending image is a native visual fixture with explicitly chosen score/time values, not a completed original-input replay.

Boundary hazards, swarm behaviour, remaining object details, item flashing, eye/dashboard animation, complete death/game-over presentation and original input replays remain incomplete. Full raster composition and cycle accuracy are not established by these checks.
...
Inner Sanctum's dog spawn enters the recovered animation record at $4e0c and pursuit preserves its special dog frames instead of entering the ordinary humanoid attack stance. Its close-contact transition similarly enters recovered record $4e2d instead of executable dispatcher $4e20. Encounter pursuit state is cleared on leaving the dog room so it cannot drive an ordinary enemy, and unresolved animation pointers now stop the actor with a visible diagnostic instead of crashing. Structural and native checks cover spawn, sustained pursuit, contact and room exit; this is not a complete original encounter replay.
...
While a projectile occupies the fourth hardware slot, the actor uses three separately editable body parts; this prevents a held weapon remaining on screen while it is in flight. There are 1,754 unique original body-part images. 300 original unmasked body compositions pass 2,764,800 GPU pixel comparisons. Ordinary non-throwing actors still use the earlier assembled sprites and mask path. Full masked body equivalence, held-weapon restoration timing, VIC priority/raster phase and complete input replays remain unverified.
