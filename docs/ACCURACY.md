...
Correction from the supplied CCS original: dog spawn uses idle record $4e08, approach boundary 13 starts $4e0c, and contact uses $4e20. These are valid data records; earlier claims describing $4e08/$4e20 as executable dispatchers were wrong. The exporter omitted JMP tail-called entries. Recovered records and bounded original-code boundary checks now cover the corrected sequence. Encounter state clearing and unresolved-pointer diagnostics remain. Updated native checks have NOT been run in GameMaker here. See [encounter follow-up](LN1_ENCOUNTER_FOLLOWUP.md).
